<template>
    <section class="section sec-9">
      <div class="bg"></div>
      <div class="img-black"></div>
      <div class="animate"></div>
      <div class="text text-1">
          <div class="title"><span>Unit Type 36/66</span></div>
      </div>
      <div class="container">
        <div class="main-wrapper">
          <div class="img-wrapper">
            <div class="text img-title">
              <div class="title">Type 36/66</div>
            </div>
    ...
    methods: {
      startAnimationStep1() {
        ...
        const AnimationSec5 = this.$scrollmagic
          .scene({
            triggerElement: ".sec-9 .container",
            triggerHook: 0.3,
            duration: "100%",
          })
          .setTween(tl);
        ...
        tl.to(".sec-9", 1, { onStart: Step1Start, onReverseComplete: Step1Reverse }, this.$parent.animationPosition());
  
        function Step1Start() {
          $('.sec-9 .img-wrapper').css('transform', 'unset').css('opacity', 1)
          $('.sec-9 .text-1').css('transform', 'unset').css('opacity', 1)
        }
  
        function Step1Reverse() {
          $('.sec-9 .img-wrapper').css('transform', 'translateY(30px)').css('opacity', 0)
          $('.sec-9 .text-1').css('transform', 'translateY(30px)').css('opacity', 0)
        }
      ...
      startAnimationStep2() {
        ...
        const AnimationSec5Step2 = this.$scrollmagic
          .scene({
            triggerElement: ".sec-9",
            triggerHook: 0.0,
            duration: "100%",
          })
          .setTween(tl).setPin('.sec-9')
  
        this.$scrollmagic.addScene(AnimationSec5Step2);
  
        this.$parent.animationPositionReset();
        tl.to(".sec-9", 1, {}, this.$parent.animationPosition());
        tl.to(".sec-9", .1, { onStart: Step2Start, onReverseComplete: Step2Reverse }, this.$parent.animationPosition(1));
  
        function Step2Start() {
          disableTouch(1000)
          $('.sec-9 .animate').css('transform', 'scale(100)').css('opacity', 1)
  
          setTimeout(()=> {
            $('.sec-9 .container').css('z-index', 3)
            $('.sec-9 .bg-main').css('opacity', 1)
            $('.sec-9 .bg-2').css('transform', 'unset').css('opacity', 1)
            $('.sec-9 .text-2').css('transform', 'unset').css('opacity', 1)
            $('.sec-9 .img-title').css('transform', 'unset').css('opacity', 1)
          }, 800)
        }
  
        function Step2Reverse() {
          disableTouch(2300)
            $('.sec-9 .animate').css('transform', 'scale(0)')
            $('.sec-9 .bg-2').css('transform', 'translateY(30px)').css('opacity', 0)
            $('.sec-9 .text-2').css('transform', 'translateY(30px)').css('opacity', 0)
            $('.sec-9 .img-title').css('transform', 'translateY(30px)').css('opacity', 0)
  
            setTimeout(()=> {
              $('.sec-9 .bg-1').css('transform', 'unset').css('opacity', 1)
              $('.sec-9 .text-1').css('transform', 'unset').css('opacity', 1)
              $('.sec-9 .animate').css('opacity', 0)
            }, 2000)
  ...
  .sec-9 {
    ...
    .img-wrapper {
      ...
      .bg-2 {
        background-image: asset.furl(pc, sec9, img2);
        transform: translateY(30px);
        transition: transform .5s, opacity .5s;
        opacity: 0;
      }
